...
export default class Posts extends Model {
  ...
  public async savePosts() {
     ...
     */
    await Promise.all(
      results.map(async (result: any, index: any) => {
...
${postMatter.content}`;
    ...
    list.sort(
      (a: any, b: any) =>
        moment(b.data.date).unix() - moment(a.data.date).unix()
    );
await this.$posts.read();
this.$posts.data['posts']=list
    this.$posts.write();
    return true;
  }
  ...
  async list() {
    ...
    const list = posts.map((post: IPostDb) => {
      ...
      item.content = helper.changeImageUrlDomainToLocal(
        item.content,
        this.appDir
      );
      item.data.feature =
        item.data.feature && !item.data.feature.includes("http")
          ? helper.changeFeatureImageUrlDomainToLocal(
              item.data.feature,
              this.appDir
            )
          : item.data.feature;
  ...
  async savePostToFile(post: IPost): Promise<IPost | null> {
    const helper = new ContentHelper();
    const content = helper.changeImageUrlLocalToDomain(
      post.content,
