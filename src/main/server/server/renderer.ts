...
export default class Renderer extends Model {
  ...
  public formatDataForRender(): any {
    ...
    this.postsData = this.db.posts
      ...
      .sort(
        (a: IPostRenderData, b: IPostRenderData) =>
  ...
  async renderTagDetail() {
    ...
    for (const usedTag of usedTags) {
      console.log(usedTag)
      console.log(this.postsData.join(""))
  ...
  async buildCss() {
    ...
    console.log(this.db.themeConfig)
    console.log(lessFilePath)
    console.log("这是lessfile")
    ...
    less.render(
      ...
      async (err: any, cssString: any /*Less.RenderOutput*/) => {
        if (err) {
          console.log(err);
        }
        let { css } = cssString;
  ...
  async buildFeed() {
    ...
    const feedFullText =
      typeof themeConfig.feedFullText === "undefined"
        ? true
