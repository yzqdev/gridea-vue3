  ...
  <div class="articles-page">
    <div class="flex justify-between tool-container">
      ...
      <div class="flex">
        ...
        />
        <el-tooltip placement="bottom" :title="$t('searchArticle')">
          <div
              class="op-btn"
        ...
        <el-tooltip placement="bottom" :title="$t('newArticle')">
...
function onSelectChange(post: any) {
  const foundIndex = state.selectedPost.findIndex(
      (item: any) => item === post
  );
  if (foundIndex !== -1) {
    state.selectedPost.splice(foundIndex, 1);
  } else {
    state.selectedPost.push(post);
  }
}
...
function close() {
  state.articleUpdateVisible = false;
  state.currentArticleFileName = "";
}

function newArticle() {
  state.articleUpdateVisible = true;
  state.currentArticleFileName = "";

  // ga.event('Post', 'Post - new', { evLabel: state.site.setting.domain })
}
function  editPost(post: IPost) {
  state.articleUpdateVisible = true;
  state.currentArticleFileName = post.fileName;
}
    ...
    onOk: () => {
      ...
      ipcRenderer.once(
          ...
          (event: IpcRendererEvent, data: any) => {
            if (data) {
              this.$message.success(t("articleDelete").toString());
    ...
    onOk: () => {
      ...
      ipcRenderer.once(
          ...
          (event: IpcRendererEvent, data: any) => {
            console.log(data);
            if (data) {
              this.$bus.$emit("snackbar-display", t("articleDelete"));
              this.$bus.$emit("site-reload");
