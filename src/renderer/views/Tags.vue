<template>
  <div class="">
    ...
    >
      ...
      >
        <el-button :style="{ marginRight: '8px' }" @click="close">
          {{ $t("cancel") }}
        </el-button>
        <el-button type="primary" :disabled="!canSubmit" @click="saveTag">{{
          $t("save")
        }}</el-button>
      </div>
    </el-drawer>
...
function checkTagValid() {
  ...
  const foundTagIndex = tags.findIndex(
    (tag: ITag) => tag.name === state.form.name || tag.slug === state.form.slug
  );
  if (foundTagIndex !== -1) {
    return false;
  }
